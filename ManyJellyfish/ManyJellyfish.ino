...
static Jellyfish jellies[] = {
  {9, 253, 3, 10, 80, 10, true, 16, 17, 2}
};
...
void setup() {
  ...
  for (int idx = 0; idx < 1; idx++) {
    ...
    jelly.strip = Adafruit_NeoPixel(numLeds, jellies[0].pin, NEO_GRB + NEO_KHZ800);
    jelly.strip.setBrightness(jelly.brightness);
    jelly.strip.begin();
    jelly.strip.show(); // Initialize all pixels to 'off'
  }
}
  ...
  for (j = 0; j < numLeds; j++) {
    for (i = 0; i < numLeds; i++) {
      ...
      for (int idx = 0; idx < 1; idx++) {
        ...
        if (pixel == jelly.drip1 || pixel == jelly.drip1 - 1 || pixel == jelly.drip2 || pixel == jelly.drip2 + 1) {
